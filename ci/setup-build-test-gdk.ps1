...
$upload_output = buildkite-agent "artifact" "upload" "testdir/test.txt" *> "output.txt"
...
$test_results_id = (Select-String -Pattern "[^ ]* testdir/test.txt" -Path "output.txt" -CaseSensitive).Matches[0].Value.Split(" ")[0]
